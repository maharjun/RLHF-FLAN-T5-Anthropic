...
class RewardMainModule(MainModule):
    ...
    def output(self, batch_dataset_struct):
        ...
        chosen_input_tokenised = self.tokenizer(chosen_input, padding=True, return_tensors='pt')
        rejected_input_tokenised = self.tokenizer(rejected_input, padding=True, return_tensors='pt')

        assert len(chosen_input_tokenised) == len(rejected_input_tokenised) == len(chosen_input) == len(rejected_input)
...
def main(cfg: DictConfig, output_paths: Paths):
    ...
    main_module: MainModule = MainModule(module_params=model_params)
    ...
    with timer("Running the Full Training Loop"):
        run_train_eval_loop(train_params=train_params,
                            dataset=dataset,
